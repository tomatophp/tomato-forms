<x-tomato-admin-layout>
    <x-slot name="header">
        {{trans('tomato-forms::global.groups.title')}}
    </x-slot>
    <x-slot name="headerBody">
        <Link href="/admin/forms"
            ...
            {{trans('tomato-forms::global.groups.back')}}
        </Link>
        <Link modal href="/admin/groups/create"
        ...
        {{trans('tomato-admin::global.crud.create-new')}} {{trans('tomato-forms::global.groups.single')}}
        </Link>
    </x-slot>
    ...
    <div class="pb-12" v-cloak>
        <div class="mx-auto">
            @if(\TomatoPHP\TomatoForms\Models\Group::count())
            <x-splade-table :for="$table" striped>
                <x-splade-cell actions>
                    <div class="flex justify-start">
                        <Link href="/admin/groups/{{ $item->id }}" class="px-2 text-blue-500" slideover>
                        <div class="flex justify-start space-x-2">
                            <x-heroicon-s-eye class="h-4 w-4 ltr:mr-2 rtl:ml-2"/>
                            <span>{{trans('tomato-admin::global.crud.view')}}</span>
                        </div>
                        </Link>
                        <Link href="/admin/groups/{{ $item->id }}/edit" class="px-2 text-yellow-400" slideover>
                        <div class="flex justify-start space-x-2">
