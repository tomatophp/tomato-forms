<x-tomato-admin-layout>
    <x-slot name="header">
        {{trans('tomato-admin::global.crud.edit')}} {{trans('tomato-forms::global.form.single')}} #{{$model->id}}
    </x-slot>
    ...
    <x-splade-form class="flex flex-col space-y-4" :unguarded="['form_title', 'description', 'name']" action="{{route('admin.forms.update', $model->id)}}" method="post" :default="$model">
        ...
        <x-tomato-repeater label="{{trans('tomato-forms::global.form.fields')}}"  :options="['field','is_primary']" name="fields" id="fields" >
