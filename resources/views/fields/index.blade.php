<x-tomato-admin-layout>
    <x-slot name="header">
        {{trans('tomato-forms::global.field.title')}}
    </x-slot>
    <x-slot name="headerBody">
        <Link href="/admin/forms"
              class="mx-4 filament-button inline-flex items-center justify-center py-1 gap-1 font-medium rounded-lg border transition-colors focus:outline-none focus:ring-offset-2 focus:ring-2 focus:ring-inset dark:focus:ring-offset-0 min-h-[2.25rem] px-4 text-sm text-white shadow focus:ring-white border-transparent bg-primary-600 hover:bg-primary-500 focus:bg-primary-700 focus:ring-offset-primary-700 filament-page-button-action">
            {{trans('tomato-forms::global.field.back')}}
        </Link>
        <Link modal href="/admin/fields/create"
        ...
        {{trans('tomato-admin::global.crud.create-new')}} {{trans('tomato-forms::global.field.single')}}
        </Link>
    </x-slot>
    ...
    <div class="pb-12" v-cloak>
        <div class="mx-auto">
            @if(\TomatoPHP\TomatoForms\Models\Field::count())
                <x-splade-table :for="$table" striped>
                    <x-splade-cell is_required>
